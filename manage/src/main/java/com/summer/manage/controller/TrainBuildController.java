...
@Controller
@RequestMapping("/trainBuild")
public class TrainBuildController {

    @Autowired
    TrainBuildService trainBuildService;
    ...
    public Map<String, Object> list(Integer teacher, Integer page, Integer limit) {
        ...
        QueryResults<TrainBuild> list = trainBuildService.findList(map.getMap(), page - 1, limit);
    ...
    @PostMapping("/save")
    @ResponseBody
    public Map<String, Object> save(@Valid TrainBuild trainBuild) {
        trainBuildService.save(trainBuild);
        return MapKit.okNoContent().getMap();
    }

    @GetMapping("/form/{id}")
    public String form(@PathVariable("id") Integer id, Model model) {
        model.addAttribute("me", trainBuildService.findOne(Long.valueOf(id)));
        return "trainBuild/add";
    }

    @PostMapping("/delete")
    @ResponseBody
    Map<String, Object> delete(@RequestBody Iterable<TrainBuild> ids) {
        trainBuildService.delete(ids);
        return MapKit.okNoContent().getMap();
    }
    ...
    public void form(HttpServletResponse response) throws IOException {
        ...
        String[] headerTitle = new String[]{"系部_#_3000", "工号_#_3000",
                "姓名_#_3000", "性别_#_3000", "职称_#_3000", "实训基地名称_#_5000",
        ...
        List<TrainBuild> list = trainBuildService.findAll();
        for (int i = 0; i < list.size(); i++) {
            Map<String, Object> map = new HashMap<>();
            TrainBuild trainBuild = list.get(i);
            map.put("0", trainBuild.getTeacher().getDepartment().getName());
            map.put("1", trainBuild.getTeacher().getJobNumber());
            map.put("2", trainBuild.getTeacher().getFullName());
            map.put("3", trainBuild.getTeacher().getSex().getName());
            map.put("4", trainBuild.getTeacher().getZc().getName());
            map.put("5",trainBuild.getName());
            map.put("6", trainBuild.getLevel().getName());
            map.put("7", trainBuild.getRelatedFileName());
